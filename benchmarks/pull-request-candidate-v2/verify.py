#!/usr/bin/env python3
"""Verify the offline Pull Request Candidate v2 benchmark."""
...
CASES_SCHEMA = "stratadiff-pull-request-candidate-cases-v2"
MANIFEST_SCHEMA = "stratadiff-pull-request-candidate-manifest-v2"
ORACLE_SCHEMA = "stratadiff-pull-request-candidate-oracle-v2"
...
EXPECTED_COVERAGE = {
    ...
    "test_merge_empty_selects_pr_head",
...
SUPERSESSION = {
    "supersedes": "pull-request-candidate-v1@1.0.0",
    "changed_case_ids": ["github-docs-empty-test-merge"],
    "previous_outcome": "inconclusive_test_merge_empty",
    "current_outcome": "selected_pr_head_test_merge_empty",
    "policy_source": {
        "authority": "GitHub Docs",
        "rule": "When the test merge commit has no status, required checks are evaluated on the head commit.",
        "url": "https://docs.github.com/en/pull-requests/how-tos/merge-and-close-pull-requests/troubleshooting-required-status-checks#conflicts-between-head-commit-and-test-merge-commit",
    },
}
...
def validate_case_provenance(case: dict[str, object], provenance: dict[str, dict[str, object]], label: str) -> None:
    ...
    if provenance_ids == ["github-docs-45788-2026-09-08"]:
        require(case["id"] == "github-docs-empty-test-merge", f"{label} docs case identity changed")
        ...
        require(observation["after"] == before, f"{label} docs target must be stable")
        require(
            observation["signals"]
            == [
                {
                    "sha": DOCS_CAPTURE["potential_merge_sha"],
                    "check_runs": {"collection": "complete", "success": 0, "other": 0},
                    "commit_statuses": {"collection": "complete", "success": 0, "other": 0},
                }
            ],
            f"{label} must preserve complete empty test-merge signal surfaces",
        )
...
def derive_case(case: dict[str, object]) -> dict[str, object]:
    ...
    selected_sha = str(after["head_sha"])
    return outcome(
        case_id,
        "selected",
        "pr_head",
...
def validate_oracle(value: dict[str, object], cases: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    docs_outcome = outcomes[case_ids.index("github-docs-empty-test-merge")]
    require(
        docs_outcome
        == outcome(
            "github-docs-empty-test-merge",
            "selected",
            "pr_head",
            str(DOCS_CAPTURE["head_sha"]),
            None,
            None,
            "test_merge_empty",
            [],
        ),
        "v2 empty-test-merge supersession outcome changed",
    )
    return outcomes
...
def validate_manifest(
    ...
    require(value["name"] == "Pull Request Candidate v2", "manifest name changed")
    ...
    require(value["supersession"] == SUPERSESSION, "v1 supersession contract changed")
    ...
    require(summary_kinds["pr_head"] >= gates["minimum_pr_head_selections"], "minimum PR-head gate failed")
...
def self_test() -> None:
    ...
    mutation(
        "v1 empty-test-merge outcome restored",
        lambda _cases, oracle: oracle["outcomes"][2].update(
            {
                "status": "inconclusive",
                "candidate_kind": None,
                "sha": None,
    ...
    weakened_supersession = copy.deepcopy(manifest_value)
    weakened_supersession["supersession"]["current_outcome"] = "inconclusive_test_merge_empty"
    expect_rejected(
        "weakened v1 supersession",
        lambda: validate_manifest(
            weakened_supersession,
            CASES_PATH.read_bytes(),
            ORACLE_PATH.read_bytes(),
            cases_value,
            validate_oracle(oracle_value, validate_cases(cases_value)),
