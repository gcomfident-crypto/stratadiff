...
The primary acquisition wedge is therefore **Pull Request Doctor**, a queue-neutral, read-only
command that works before an App install and produces an exact-candidate diagnostic bundle from
still-observable GitHub evidence. A qualified owner can next run an administration-scoped but
non-mutating **Merge Readiness Preflight**. Only repeated use of those two surfaces, plus a verified
need to retain a candidate chain that current public evidence no longer exposes for the team's
incidents, justifies asking for a lightweight **App Recorder**. The recorder preserves event-time
candidate, delivery, Check, and policy observations; it does not become a merge gate merely because
it was installed. A shadow or required **Review
Evidence Control Plane** follows only after the retained records prove the need and survive live
replay. Review Cache and Review Resume remain optimization and recovery surfaces, not merge
authority.
...
| [`gh pr checks --required`](https://cli.github.com/manual/gh_pr_checks) and the current CLI [query](https://github.com/cli/cli/blob/0d121e8c31204cdbe02b4a4b1be5e88bdc3c4662/api/query_builder.go#L270), accessed 2026-09-08 | The native command already filters required checks and exports workflow and event fields. Its query reads `commits(last: 1)` for the PR; its output does not join expected and observed App identities or discover a separately created merge-group candidate. | Do not claim required-check listing is missing. Doctor must add an evidence-backed cross-layer join and must say `unknown` when it cannot identify the candidate or producer. |
| GitHub [Rules Insights documentation](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/managing-rulesets-for-a-repository#viewing-insights-for-rulesets) and [2026-08-25 GA announcement](https://github.blog/changelog/2026-08-25-rule-insights-dashboard-generally-available/) | GitHub already exposes repository and organization rule pass, fail, bypass, and Evaluate results, with drill-down and CSV export. Ruleset history records evaluation around a ref update or merge attempt; it is not a prospective workflow-producer or trigger audit. | **No-Go:** do not sell another rules dashboard. Use the rule result as one link in context/App → candidate SHA → Check/suite/run/job → workflow-trigger evidence. |
...
| [dcoapp/app #303](https://github.com/dcoapp/app/issues/303), resolved 2026-07-29 | One PR sat in queue for about an hour with required `DCO` still `Expected`, then GitHub ejected it. By the later investigation, that incident's current public timeline no longer exposed the exact candidate SHA and its temporary ref and Check Runs were gone, so the SHA → Check chain could not be reconstructed from public evidence. App-side registration and delivery evidence later established both missing `merge_queues: read` and a live registration subscribed to `merge_queue_entry` instead of `merge_group`. | A post-hoc Doctor may prove only that the expected producer did not emit on an observed candidate whose identity was retained; it must not infer the provider-internal cause or claim an exact SHA it can no longer recover. This is evidence for a prospective recorder only after repeated post-hoc diagnoses show that ephemeral-object loss matters to the team. |
...
| [dcoapp/app #303](https://github.com/dcoapp/app/issues/303) | The required `DCO` signal never appeared and the candidate was evicted. For that old event, the current public timeline no longer exposed the exact candidate SHA and the temporary ref and Check Runs were gone. The permission/subscription defect was learned from App-side evidence, not reconstructed from the old candidate. | Use this as an abstention control: report only the candidate identity and missing producer signal actually retained at observation time, state when the exact historical chain is unavailable, and never rewrite the later App-side diagnosis as something Doctor could have known. |
...
| GitHub CLI and Rules Insights | [`gh pr checks --required`](https://cli.github.com/manual/gh_pr_checks) already lists required checks and exports workflow and event fields. Rules Insights shows rule outcomes, bypasses, and Evaluate results. The current CLI query reads the PR's last commit rather than reconstructing a native merge-group candidate, and the output does not join expected and observed App IDs. | Do not claim that GitHub lacks blocker visibility. Doctor must add the queue-neutral policy → candidate → producer → workflow join, and only for evidence it can actually observe. |
| Mergify | [`mergify queue show <PR>`](https://docs.mergify.com/merge-queue/monitoring/) exposes queue position, priority, CI summary, and blocking conditions. Mergify also documents [App-qualified Check conditions](https://docs.mergify.com/changelog/2026-06-02-scope-check-conditions-to-a-specific-github-app/). Its [2026-07-24 waiting display](https://docs.mergify.com/changelog/2026-07-24-see-what-a-queued-pull-request-is-waiting-on/) is specifically about `Depends-On` relationships, not a universal reason engine. | Do not claim CLI blocker explanation or App-qualified checks are unique. Those capabilities are tied to a Mergify installation and queue; Doctor's narrower opening is pre-install diagnosis across GitHub native and third-party merge paths. |
| Aviator merge queue | Aviator documents blocker reasons in its [sticky comment](https://docs.aviator.co/mergequeue/concepts/sticky-comments), exposes [`av pr status`](https://docs.aviator.co/aviator-cli/manpages/av-pr-status-1), and can list [pending workflows that never started](https://docs.aviator.co/mergequeue/concepts/pending-workflow-runs). That workflow diagnosis is scoped to Aviator's queue and GitHub Actions and does not distinguish required from non-required workflows. | Do not claim that queue vendors cannot explain missing work. StrataDiff must work before queue migration and preserve the required-policy distinction. |
| Trunk merge queue | Trunk's [testing-details API](https://docs.trunk.io/merge-queue/reference/merge/get-details-about-testing-that-merge-queue-is-performing) exposes required-status sources, the exact `testBranchSha`, Checks, and tested PRs for work performed by Trunk. | Exact candidate/check chains are not an industry-wide blank space. The remaining claim is a queue-neutral, pre-install join that also handles expected App identity and workflow-trigger evidence without pretending to see inside the queue vendor. |
...
`integration_id`. This internal technical proof does not waive the public repeat-use gate. Offer
recorder and then shadow mode on consenting repositories only after their owners verify and reuse
Doctor or Preflight and confirm an ephemeral-evidence need. Use real Copilot and CodeRabbit evidence
where available and preserve the complete event, evidence, policy-generation, candidate, override,
Check, and repair ledger. Exercise at least 80 cases: push during review,
